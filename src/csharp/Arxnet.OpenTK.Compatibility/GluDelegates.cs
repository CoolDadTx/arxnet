/*
 * Copied from Compatibility of OpenTK that doesn't seem to be in NuGet
 * https://github.com/mono/opentk/blob/master/Source/Compatibility/Graphics/Glu/
 */
...
#pragma warning restore IDE0065 // Misplaced using directive
    ...
    //P3NET: Not going to do this complex solution for loading delegates, don't care about cross platform at this point, just load delegates normally    
    partial class Glu
    {
        internal static partial class Delegates
        {
            [System.Security.SuppressUnmanagedCodeSecurity()]
            [DllImport("glu32.dll", CallingConvention = CallingConvention.Winapi, ExactSpelling = true, SetLastError = false, EntryPoint = "gluBuild2DMipmaps")]
            public extern static int gluBuild2DMipmaps ( TextureTarget target, int internalFormat, int width, int height, PixelFormat format, PixelType type, IntPtr data );
            //internal delegate int Build2DMipmaps ( TextureTarget target, int internalFormat, int width, int height, PixelFormat format, PixelType type, IntPtr data );
            //internal static Build2DMipmaps gluBuild2DMipmaps;

            [System.Security.SuppressUnmanagedCodeSecurity()]
            [DllImport("glu32.dll", CallingConvention = CallingConvention.Winapi, ExactSpelling = true, SetLastError = false, EntryPoint = "gluPerspective")]
            public extern static void gluPerspective ( double fovy, double aspect, double zNear, double zFar );
