...
namespace P3Net.Arx
{
	...
	//==============================================================================
		...
		public static void LyricColour(int r, int g, int b, int a)
		{
	...
	//------------------------------------------------------------------------------
		public static void LoadLyrics(string filename)
		{
			...
			string line; // *** LINE stores the current string
			string left;
			string right;
			...
			while (line != "EOF")
			{
				...
				string.size_type idx = new string.size_type(); // *** string::size_type is an integer datatype large enough to represent any possible string size
				getline(instream, line); // *** read first line as blank
				idx = line.IndexOf(','); // *** yields the index in our LINE of the ','.
