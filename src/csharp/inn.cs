...
namespace P3Net.Arx
{
	public class InnRoom
	{
		...
		public int fatigueRecoveryProbability { get; set; }
	...
	public partial class GlobalMembers
	{
			...
			while (InnMenu > 0)
			{
				...
				while (InnMenu == 2) // first room booking menu
				{
					...
					for (int i = 0 ; i < 5 ; i++) // Max number of room prices in this menu display
					{
						x = 30;
						//MLT: Downcast to int
						room_cost = (int)(Rooms[i].baseCost * Inns[InnNo].costMultiplier);
						if (room_cost < 100)
							x = 31;
						if (room_cost > 999)
							x = 29;
				...
				while (InnMenu == 4) // Confirm room choice and rate
				{
					...
					//MLT: Downcast to int
					str = Rooms[roomChoice].name + " is " + ToCurrency((int)(Rooms[roomChoice].baseCost * Inns[InnNo].costMultiplier)) + " coppers.";
					CyText(1, str);
		...
		{
			new InnRoom() { name = "the common area floor", baseCost = 10, fatigueRecoveryProbability = 30 },
			new InnRoom() { name = "a bed with no bath", baseCost = 20, fatigueRecoveryProbability = 45 },
			new InnRoom() { name = "a bed with common bath", baseCost = 30, fatigueRecoveryProbability = 60 },
			new InnRoom() { name = "a room with common bath", baseCost = 40, fatigueRecoveryProbability = 85 },
			new InnRoom() { name = "a room with bath", baseCost = 50, fatigueRecoveryProbability = 100 },
			new InnRoom() { name = "a Premium Room", baseCost = 100, fatigueRecoveryProbability = 140 },
			new InnRoom() { name = "a Deluxe Room", baseCost = 200, fatigueRecoveryProbability = 170 },
			new InnRoom() { name = "a Small Suite", baseCost = 400, fatigueRecoveryProbability = 200 },
			new InnRoom() { name = "a Suite", baseCost = 800, fatigueRecoveryProbability = 240 },
			new InnRoom() { name = "our BEST Suite", baseCost = 1600, fatigueRecoveryProbability = 255 }
		};

		public static Inn[] Inns =
		{
			new Inn() { name = "Green Boar Inn", costMultiplier = 1, jobProbability = 64, x = 9, y = 44 },
			new Inn() { name = "Lazy Griffin Inn", costMultiplier = 2, jobProbability = 154, x = 33, y = 11 },
			new Inn() { name = "Sleeping Dragon Inn", costMultiplier = 0.5, jobProbability = 64, x = 60, y = 57 },
			new Inn() { name = "Traveller's Inn", costMultiplier = 6, jobProbability = 179, x = 32, y = 39 },
			new Inn() { name = "Midnight Inn", costMultiplier = 3, jobProbability = 56, x = 32, y = 40 },
			new Inn() { name = "Warrior's Retreat", costMultiplier = 0.75, jobProbability = 77, x = 28, y = 9 },
			new Inn() { name = "Royal Resort Inn", costMultiplier = 8, jobProbability = 102, x = 31, y = 60 }
		};
		...
		{
			new InnJob() { name = "Bellhop", minIncome = 10, maxIncome = 24, statRequirementName = "Strength", statRequirementValue = 18, fatigueRate = 0.6875F, minorWoundProbability = 15.95F, majorWoundProbability = 3.97F },
			new InnJob() { name = "Desk Clerk", minIncome = 18, maxIncome = 20, statRequirementName = "Charm", statRequirementValue = 16, fatigueRate = 0.5625F, minorWoundProbability = 3.75F, majorWoundProbability = 0.15F },
			new InnJob() { name = "Janitor", minIncome = 12, maxIncome = 15, statRequirementName = "Stamina", statRequirementValue = 17, fatigueRate = 0.6250F, minorWoundProbability = 8.50F, majorWoundProbability = 0.88F }
		};
